...
type StaticPermission = {
  [K in Permission["type"]]: Extract<Permission, { type: K }> extends {
    type: K
  } & infer Rest ? keyof Rest extends never ? Extract<Permission, { type: K }> : never : never;
}[Permission["type"]];

function isStaticPermission(permission: Permission): permission is StaticPermission {
  return Object.keys(permission).length === 1;
}

export async function checkPermission(session: UserSession, permission: Permission): Promise<boolean> {
