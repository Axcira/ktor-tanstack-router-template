...
function DetailPage() {
  ...
  const [notes, setNotes] = useState("既存のレガシーシステムをモダンなWebアプリケーションに移行するプロジェクト。\nフェーズ1: 要件定義・設計\nフェーズ2: 開発・テスト\nフェーズ3: 移行・運用開始")
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-1 border rounded-md p-0.5">
            {(["loaded", "loading", "error"] as const).map((s) => (
              <Button key={s} variant={viewState === s ? "secondary" : "ghost"} size="xs" onClick={() => setViewState(s)}>
                {s === "loaded" ? "通常" : s === "loading" ? "読込中" : "エラー"}
              </Button>
            ))}
          </div>
        </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <Card className="border-destructive/30">
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="font-medium text-destructive">案件データの取得に失敗しました</p>
            <p className="text-sm text-muted-foreground mt-1">指定された案件が存在しないか、アクセス権限がありません</p>
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Master-Detail 編集画面</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex gap-1 border rounded-md p-0.5">
            {(["loaded", "loading", "error"] as const).map((s) => (
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="startDate" className="text-xs">開始日</Label>
                <Input id="startDate" type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="endDate" className="text-xs">終了日</Label>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-3 text-sm">
                {[
                  { action: "ステータスを「進行中」に変更", user: "田中太郎", date: "2026-06-28" },
                  { action: "明細を更新", user: "佐藤花子", date: "2026-06-25" },
                  { action: "案件を作成", user: "田中太郎", date: "2026-04-01" },
                ].map((log, i) => (
