...
function ArticleDetail({article}: { article: ArticleDTO }) {
  ...
  const {isAllowed: isEditAllowed} = useAuthorize({type: "UpdateArticle", allowOthers: article.userId !== session?.user.id})
  const {isAllowed: isDeleteAllowed} = useAuthorize({type: "DeleteArticle", allowOthers: article.userId !== session?.user.id})

  return <div className="container mx-auto p-6 max-w-4xl space-y-8">
    ...
    <article className="space-y-6">
      ...
      <div className="flex gap-2">
        {isEditAllowed && (<Button asChild variant="outline" size="sm">
          <Link
