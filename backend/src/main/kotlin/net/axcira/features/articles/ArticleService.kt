...
private fun upsertTags(tags: List<String>) =
    run {
...
class ArticleService(
    ...
    suspend fun get(pagination: Pagination): List<ArticleDTO> =
        database.dbQuery {
            val articles = Articles.selectAll().paginate(pagination).toList()
            val tagsByArticle =
                (ArticleTags innerJoin Tags)
                    .selectAll()
                    .where { ArticleTags.articleId inList articles.map { it[Articles.id] } }
    ...
    suspend fun update(
        ...
        database.dbQuery {
            ...
            val resultTags =
                tagList.getOrNull()?.let { tags ->
                    val currentTags =
                        (ArticleTags innerJoin Tags)
                            .selectAll()
                            .where { ArticleTags.articleId eq id }
            ...
            return@dbQuery Articles
                .updateReturning(where = { Articles.id eq id }) {
